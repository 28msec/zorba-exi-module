                                          ...
                                          const zorba::DynamicContext* aDynamicContext) const
{
  ...
  try {
    ...
    //fclose(fxml);
    //end debug
    StaticContext_t lSctx = Zorba::getInstance(0)->createStaticContext();
    Item    parse_xml_name;
    if(!is_fragment)
    ...
    if(is_fragment)
    {
      ...
      lSctx->loadProlog("import module namespace fn-zorba-xml = "
                       "\"http://www.zorba-xquery.com/modules/xml\";\n"
                       "import schema namespace opt = "
                       "\"http://www.zorba-xquery.com/modules/xml-options\";",
                       compiler_hints);
    }
    ItemSequence_t  xmldoc_seq = lSctx->invoke(parse_xml_name, parse_xml_args);
    return xmldoc_seq;
